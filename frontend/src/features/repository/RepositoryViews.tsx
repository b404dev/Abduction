...
type ReviewAction = "comment" | "approve" | "request-changes";

function PullRequestPanel({ repositoryPath, detail, loading, onError }: { repositoryPath: string; detail: PullRequestDetail | null; loading: boolean; onError: (message: string) => void }) {
  ...
  function submitReview() {
    ...
    setSubmitting(true);
    api.submitPullRequestReview(repositoryPath, detail!.number, pendingAction, reviewBody).then(() => {
      setSubmittedMessage(pendingAction === "approve" ? "Approval submitted." : pendingAction === "request-changes" ? "Change request submitted." : "Review comment submitted.");
      setReviewBody("");
      setPendingAction(null);
    }).catch((reason: unknown) => onError(String(reason))).finally(() => setSubmitting(false));
  }
  const actionLabel = pendingAction === "approve" ? "Approve" : pendingAction === "request-changes" ? "Request changes" : "Comment";
  return <aside className="pr-detail"><header><div><span className="eyebrow">PR #{detail.number} · @{detail.author}</span><h3>{detail.title}</h3></div><button className="ghost" onClick={() => api.openURL(detail.url)}>GitHub ↗</button></header><div className="pr-metrics"><span><b className="diff-add">+{detail.additions}</b> additions</span><span><b className="diff-delete">−{detail.deletions}</b> deletions</span><span><b>{detail.changedFiles}</b> files</span><span><b>{detail.commits}</b> commits</span></div><div className="pr-decision"><span>{detail.reviewDecision || "Review pending"}</span><span>{detail.mergeable.toLowerCase()}</span></div>{detail.body ? <p className="pr-body">{detail.body}</p> : null}<section className="pr-files"><h4>Changed files</h4>{detail.files.map((file) => <div key={file.path}><code>{file.path}</code><span className="diff-add">+{file.additions}</span><span className="diff-delete">−{file.deletions}</span></div>)}</section>{detail.state === "OPEN" ? <section className="pr-review"><h4>Submit review</h4><textarea value={reviewBody} onChange={(event) => setReviewBody(event.target.value)} placeholder="Leave review feedback…" rows={4}/>{submittedMessage ? <p role="status">{submittedMessage}</p> : null}<div className="pr-review__actions"><button className="ghost" disabled={!reviewBody.trim()} onClick={() => setPendingAction("comment")}>Comment</button><button className="primary" onClick={() => setPendingAction("approve")}>Approve</button><button className="ghost danger" disabled={!reviewBody.trim()} onClick={() => setPendingAction("request-changes")}>Request changes</button></div></section> : null}<section className="pr-diff"><h4>Unified diff</h4><pre>{detail.diff.split("\n").map((line, index) => <span className={diffLineClass(line)} key={`${index}-${line.slice(0, 20)}`}>{line || " "}</span>)}</pre></section>{pendingAction ? <div className="review-confirm" role="dialog" aria-modal="true" aria-label="Confirm pull request review"><section><span className="eyebrow">GitHub write action</span><h3>{actionLabel} on PR #{detail.number}?</h3><p>{reviewBody || "No review message."}</p><footer><button className="ghost" disabled={submitting} onClick={() => setPendingAction(null)}>Cancel</button><button className={pendingAction === "request-changes" ? "ghost danger" : "primary"} disabled={submitting} onClick={submitReview}>{submitting ? "Submitting…" : `Confirm ${actionLabel.toLowerCase()}`}</button></footer></section></div> : null}</aside>;
}
