...
import { ArrowDownToLine, BarChart3, BrainCircuit, Braces, CloudDownload, ExternalLink, GitBranch, GitPullRequestArrow, Keyboard, Palette, ScrollText, Search, Settings2, ShieldCheck, SquarePen, UserRound, Wrench, type LucideIcon } from "lucide-react";
import { api } from "../../api";
import type { GitIdentity, Repo, RepositorySources, ViewName } from "../../types";
import { fuzzyFilter } from "../../search";

export const destinations: { name: ViewName; label: string; short?: string; key: string; icon: LucideIcon }[] = [
  ...
  { name: "tools", label: "Dependencies", short: "Tools", key: "7", icon: Wrench }, { name: "themes", label: "Themes", key: "8", icon: Palette }, { name: "logs", label: "Logs", key: "9", icon: ScrollText }, { name: "settings", label: "Settings", key: "0", icon: Settings2 },
];

// modifierLabel names the platform's command modifier for on-screen shortcut hints.
...
type TitlebarProps = {
  platform: string; repos: Repo[]; selectedRepo: Repo | null;
  onSelect: (repo: Repo) => void; onCloned: (repo: Repo) => void; onClone: (repository: Repo) => Promise<void>; onRefresh: () => Promise<void>;
  onCommand: () => void; onShortcuts: () => void; onError: (message: string) => void;
};

// Titlebar holds the brand, the repository and branch context, and every repository-level action.
export function Titlebar({ platform, repos, selectedRepo, onSelect, onCloned, onClone, onRefresh, onCommand, onShortcuts, onError }: TitlebarProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    api.gitIdentity(selectedRepo?.path ?? "").then((identity) => { if (!cancelled) setGitIdentity(identity); }).catch(() => { if (!cancelled) setGitIdentity(null); });
  ...
  function chooseBranch(branch: string) {
    if (!selectedRepo || !branch || branch === selectedRepo.branch) return;
    if (!selectedRepo.path) { onSelect({ ...selectedRepo, branch }); return; }
    setSwitchingBranch(true);
    api.switchBranch(selectedRepo.path, branch).then((resolvedBranch) => onSelect({ ...selectedRepo, branch: resolvedBranch })).catch((reason: unknown) => onError(String(reason))).finally(() => setSwitchingBranch(false));
  }
...
function RepositoryActions({ repo, onClone, onRefresh, onError }: { repo: Repo | null; onClone: (repository: Repo) => Promise<void>; onRefresh: () => Promise<void>; onError: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    const clearTimer = window.setTimeout(() => setPullStatus(""), 4000);
    return () => window.clearTimeout(clearTimer);
  }, [pullStatus]);

  // pullLatest fast-forwards the checkout and refreshes every repository-derived view.
  function pullLatest() {
  ...
  if (!repo) return null;
  if (!repo.path) {
    return <button className="primary" disabled={cloningRemote} onClick={() => { setCloningRemote(true); onClone(repo).catch((reason: unknown) => onError(String(reason))).finally(() => setCloningRemote(false)); }}><CloudDownload size={14}/><span className="action-label">{cloningRemote ? "Cloning…" : "Clone to workspace"}</span></button>;
  }
  return <>
    {pullStatus ? <span className="pull-status" role="status">{pullStatus}</span> : null}
    <button className="primary pull-action" disabled={pulling} onClick={pullLatest} title="Pull the latest changes"><ArrowDownToLine size={14}/><span className="action-label">{pulling ? "Pulling…" : "Pull latest"}</span></button>
    <button className="ghost editor-action" onClick={() => api.openInEditor(repo.path, "")} title="Open repository in your editor (E)"><SquarePen size={14}/><span className="action-label">Editor</span></button>
    {repo.githubUrl ? <button className="icon-button github-action" onClick={() => api.openOnGitHub(repo)} aria-label="Open repository on GitHub" title="Open on GitHub (G)"><ExternalLink size={14}/></button> : null}
  </>;
}
...
// AlienGlyph is the saucer-and-beam mark used by the brand, loading, and empty states.
...
// Rail is a fixed-width strip of labelled destinations; it never expands or overlays content.
export function Rail({ view, onView, errorCount, version }: { view: ViewName; onView: (view: ViewName) => void; errorCount: number; version?: string }) {
  return <nav className="rail" aria-label="Primary navigation">{destinations.map((destination) =>
    <button key={destination.name} className={view === destination.name ? "rail__item rail__item--active" : "rail__item"} onClick={() => onView(destination.name)} aria-label={`${destination.label} (${destination.key})`} aria-current={view === destination.name ? "page" : undefined} title={`${destination.label} · press ${destination.key}`}><destination.icon className="rail__icon" strokeWidth={1.7}/><span className="rail__label">{destination.short ?? destination.label}</span>{destination.name === "logs" && errorCount ? <b className="rail__badge">{Math.min(errorCount, 99)}</b> : null}</button>)}<div className="rail__spacer"/>{version ? <span className="rail__version" title={`Abduction v${version}`}>v{version}</span> : null}</nav>;
}
