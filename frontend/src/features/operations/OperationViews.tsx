...
export function SecurityView({ repo, onError }: { repo: Repo; onError: (message: string) => void }) {
  ...
  function startScan() { if (!selectedScanner) return; setOutput([`Starting ${selectedScanner}...`]); setReportPath(""); setRunning(true); api.startScan(repo.path, selectedScanner).then(setJobID).catch((reason: unknown) => { setRunning(false); onError(String(reason)); }); }
